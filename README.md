...
**该项目是将XDP移植至安卓平台，并进一步通过XDP在安卓上实现网络处理相关的应用。**XDP是linux内核中一个强大、可编程、高效的网络数据通路。XDP在网络栈的最底层，以软件的方式实现了包处理和过滤，能运行在裸金属机器上。它在保持linux原本网络栈的情况下，提供了高效可编程的解决方案，相当于在linux网络栈中增加了预处理的一层。
